...
def handle_audio_stream(data):
    try:
        ...
        if random.randint(1, 50) == 1:
            ...
            logger.info(f"Audio packet: type={data_type}, size={data_size} bytes")
        ...
        if isinstance(data, str):
            ...
            try:
                ...
                logger.info(f"Successfully decoded base64 string data, size={len(binary_data)} bytes")
            except Exception as e:
            ...
            if random.randint(1, 100) == 1:
                logger.info(f"Received binary data directly, size={len(binary_data)} bytes")
        elif hasattr(data, 'read'):
            # File-like object
            binary_data = data.read()
            logger.info(f"Read data from file-like object, size={len(binary_data)} bytes")
        else:
            # Unknown format
            logger.error(f"Unsupported data format: {type(data)}")
            socketio.emit('deepgram_error', {'error': 'Unsupported audio format'}, room=session_id)
            return
        ...
        try:
            ...
            if random.randint(1, 200) == 1:
                logger.info(f"Successfully sent {len(binary_data)} bytes to Deepgram")
        except Exception as e:
            logger.error(f"Error sending data to Deepgram: {e}")
            socketio.emit('deepgram_error', {'error': f'Error sending audio: {str(e)}'}, room=session_id)
